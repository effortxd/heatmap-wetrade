...
export default function SectionTime({ rows }) {
  ...
  return (
    <div className="card">
      ...
      {!rows?.length ? (
        <div className="empty">Add data-section to your sections</div>
      ) : (
        <table className="tbl">
          <thead><tr><th>Section</th><th style={{ width: 200 }}>Avg time</th><th style={{ width: 80, textAlign: "right" }}>Avg</th><th style={{ width: 80, textAlign: "right" }}>Views</th></tr></thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.section_name}>
                <td className="lead"><span className="tag">{r.section_name}</span></td>
                <td><div className="bar-cell"><span style={{ width: (r.avg_duration_ms / max) * 100 + "%" }} /></div></td>
